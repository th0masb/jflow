...
/**
 * Abstraction of iterable object which can construct enhanced iterators ({@link
 * RichIterator}).
 *
...
{
    /**
     * Constructs an iterator traversing the elements encapsulated by this
     * object.
     *
     * @return See above.
     */
     ...
     * @return The first element to pass the predicate test if there is one,
     * nothing otherwise.
     */
    ...
    /**
     * Finds the minimum element in the iterator created by {@link #iter()}
     * according to the given comparator throwing an exception if the iterator
     * is empty.
     *
     * @param orderingFunction the complete ordering of this element type.
     * @return see {@link RichIterator#minOp(Comparator)}.
    ...
    /**
     * Calculates whether every element in the iterator created by {@link
     * #iter()} passes the supplied predicate.
     *
     * @param condition The predicate test.
     * @return true if all elements pass the test or the iterator is empty,
     * false otherwise.
     */
     ...
     */
    /**
     * Calculates at least one element in the iterator created by {@link
     * #iter()} passes the supplied predicate.
     *
     * @param condition The predicate test.
    ...
    /**
     * Calculates whether all elements in the iterator created by {@link
     * #iter()} fail the supplied predicate.
     *
     * @param condition The predicate test.
     * @return true if all elements fail the test or the iterator is empty,
     * false otherwise.
     */
    ...
    /**
     * Groups the elements of the iterator created by {@link #iter()} according
     * to some classification function.
     *
    ...
    /**
     * Folds the elements of the iterator created by {@link #iter()} into one
     * value using a provided reduction operator and a starting value.
     *
     * @param <R>     The type of the fold result.
     * @param id      The initial value of the fold operation.
     * @param reducer The reduction operator governing how two values are
     *                combined into one.
    ...
    /**
     * Folds the elements of the iterator created by {@link #iter()} into one
     * value using a given reduction function making an allowance for a
     * potentially empty source.
     *
     * @param reducer The reduction operator governing how two values are
     *                combined into one.
    ...
    /**
     * Folds the elements of the iterator created by {@link #iter()} into one
     * value using a given reduction function throwing an exception if the
     * source is empty.
     *
     * @param reducer The reduction operator governing how two values are
     *                combined into one.
    ...
    /**
     * Builds a Map from the elements in the iterator created by {@link #iter()}
     * by applying two functions to each value.
     *
    ...
    /**
     * Builds a Map from the elements in the iterator created by {@link #iter()}
     * by associating each with it's image under the provided function.
     *
     * @param <V>      the type of the values in the constructed map.
     * @param valueMap The function determining the map values.
    ...
    /**
     * Builds an ArrayList from the elements in the iterator created by {@link
     * #iter()}.
     *
    ...
    /**
     * Builds an HashSet from the elements in the iterator created by {@link
     * #iter()}.
     *
    ...
    /**
     * Builds a collection from the elements in the iterator created by {@link
     * #iter()}.
     *
    ...
    /**
     * Creates a new instance of the given type by applying the given collector
     * to the iterator returned by {@link #iter()}.
     *
     * @param <R>               The target type of the transformation.
     * @param iteratorCollector The collector which determines how this vector
     *                          will be transformed.
