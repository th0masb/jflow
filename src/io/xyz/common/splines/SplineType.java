...
public enum SplineType implements ISplineConstructor
{
	...
	SHARP
	{
		...
		public ISpline constructFrom(final IPolyEdge edge)
		{
			...
			final List<ISplineSegment> lineSegments = IntStream.range(0, constituents.size() - 1)
					.mapToObj(i -> new StraightSegment(constituents.get(i), constituents.get(i+1)))
	...
	CUSTOM_SUGIYAMA
	{
		...
		public ISpline constructFrom(final IPolyEdge edge)
		{
			...
			final List<Point3D> constituents = edge.getLayoutPoints().getKnotPoints();
			final List<StraightSegment> sharp = IntStream.range(0, constituents.size() - 1)
					.mapToObj(i -> new StraightSegment(constituents.get(i), constituents.get(i+1)))
					.collect(Collectors.toList());

			final List<StraightSegment> scaledStraights = sharp.stream().map(x -> x.scale(scaleF)).collect(Collectors.toList());
			...
			for (int i = 0; i < nJoins; i++)
			{
				...
				quadJoins.add(new QuadraticBezier(start, ctrl, end));//new StraightSegment(start, end));//);
			}

			final List<ISplineSegment> finalSegments = new ArrayList<>(2 + nJoins + scaledStraights.size());
			finalSegments.add(new StraightSegment(sharp.get(0).getStart(), scaledStraights.get(0).getStart()));
			for (int i = 0; i < nJoins; i++)
			{
			...
			finalSegments.add(new StraightSegment(scaledStraights.get(nJoins).getEnd(), sharp.get(nJoins).getEnd()));
	...
	PIECEWISE_CUBIC_BEZIER
	{
		 ...
		 * The idea here is:
		 *
		 * Let E be our edge in question which has Knot points K0,....,Kn (endpoints included).
		 * (NOTE- the curve is assumed to not be closed!! i.e K0 != Kn)
		 *
		 * We would like to make a curve C formed of n cubic bezier curves which satisfies:
		 * 	- C is smooth (twice continuously differentiable)
		 * 	- C passes through each knot point at least once.
		 *
		 * In order to calculate the control points for each constituent curve we can form a
		 * tri diagonal system of simultaneous equations arising from the smoothness constraints
		 * and apply the Thomas algorithm to solve. We note that boundary conditions are required
		 * to complete the system and so we say that we would like to the curve to be linear at
		 * both endpoints (here is where we assume curve not closed).
		 *
		 * Definition of cubic bezier curve is: ((1-t)^3)P0 + (3t(1-t)^2)P1 + (3(1-t)t^2)P2 + (t^3)P3
		 *
		 * differentiating and equating we obtain the system in terms of the unknown P_j where we define
		...
		public ISpline constructFrom(final IPolyEdge edge)
		{
			 ...
			 * algorithm (good article on wikipedia). We do this for both x and y coordinates
			 * of the first control points.
			 */
			final Point3D[] K = edge.getLayoutPoints().getKnotPoints().toArray(new Point3D[0]);
