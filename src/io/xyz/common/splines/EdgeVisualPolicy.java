...
import org.lhasalimited.common.graph.IGraph;
...
import org.lhasalimited.common.serialization.impl.DataStreamSerializationException;
...
public class EdgeVisualPolicy implements IDataStreamSerializable
{
	 ...
	 * The policy mapping
	 */
	private final Map<Class<? extends IPolyEdge>, SplineType> policies = new HashMap<>();
	...
	public void putPolicy(final Class<? extends IPolyEdge> edgeClazz, final SplineType policy)
	{
	 ...
	 *
	 */
	public EdgeVisualPolicy(final Map<Class<? extends IPolyEdge>, SplineType> policies)
	{
		this.policies.putAll(policies);
	}

	public void assignEdgeVisualPolicy(final IGraph<?, ? extends IPolyEdge> graph)
	{
		for (final IPolyEdge edge : graph.getEdges())
		{
			...
			{
				throw new IllegalArgumentException("No policy for this edge class: " + edge.getClass().getSimpleName());
			}
	...
	@Override
	public void fromDataStream(final IDataStreamReader reader) throws DataStreamSerializationException
	{
		try
		{
			//			try
			//			{
			final String[] clazzArr = reader.readStringArray(), splineArr = reader.readStringArray();

			for (int i = 0; i < clazzArr.length; i++)
			{
				putPolicy((Class<? extends IPolyEdge>) Class.forName(clazzArr[i]), SplineType.valueOf(splineArr[i]));
			}
			//			}
	...
	@Override
	public void toDataStream(final IDataStreamWriter writer) throws DataStreamSerializationException
	{
		try
		{
			final List<Map.Entry<Class<? extends IPolyEdge>, SplineType>> policies = new ArrayList<>(this.policies.entrySet());
			final int npolicy = policies.size();
			final String[] clazzArr = new String[npolicy], splineArr = new String[npolicy];
