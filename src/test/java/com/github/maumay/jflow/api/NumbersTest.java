...
public class NumbersTest extends AbstractSourceTest<AbstractLongIterator>
{
    ...
    {
        assertEquals(IntVec.of(0, 1, 2, 3, 4),
                Numbers.natural().take(5).toVec());
    }

    @Test
    void testFibonnaciNumbers()
    {
        assertEquals(LongVec.of(1, 1, 2, 3, 5),
                Numbers.fibonacci().take(5).toVec());
        assertEquals(LongVec.of(2, 2, 4, 6, 10),
                Numbers.fibonacci(2, 2).take(5).toVec());
    }
