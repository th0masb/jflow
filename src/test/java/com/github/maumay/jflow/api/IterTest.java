...
class IterTest
{
    ...
    void testFlatten()
    {
        assertEquals(vec("a", "b", "c"),
                Iter.flatten(vec(vec(), vec("a"), vec(), vec("b", "c")))
                        .toVec());
    }
    ...
    {
        assertEquals(vec("a"),
                Iter.wrap(Arrays.asList("a").iterator()).toVec());
    }
    ...
    {
        assertEquals(vec("a"),
                Iter.over(() -> Arrays.asList("a").iterator()).toVec());
    }
    ...
    void testPartition()
    {
        ...
        assertThrows(IllegalArgumentException.class,
                () -> Iter.partition(0, 1, 0));
    }

    @Test
    void testApply()
    {
        assertEquals(vec("a", "aa", "aaaa"),
                Iter.apply(x -> x + x, "a").take(3).toVec());
    }
    ...
    void testRepeatAndCall()
    {
        ...
        assertEquals(IntVec.of(1, 1, 1),
                Iter.callInts(() -> 1).take(3).toVec());
        assertEquals(LongVec.of(1, 1, 1),
                Iter.callLongs(() -> 1).take(3).toVec());
        assertEquals(DoubleVec.of(1, 1, 1),
                Iter.callDoubles(() -> 1).take(3).toVec());
    }
    ...
    void testIndex()
    {
        ...
        assertEquals(IntVec.of(0, 2, 4),
                Iter.indexInts(n -> 2 * n).take(3).toVec());
        assertEquals(LongVec.of(0, 2, 4),
                Iter.indexLongs(n -> 2 * n).take(3).toVec());
