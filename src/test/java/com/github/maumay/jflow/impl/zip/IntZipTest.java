 ...
 *
 */
public final class IntZipTest extends
        AbstractIntAdapterTest<AbstractRichIterator<IntTup>>
{
    @Override
    protected List<Case<AbstractRichIterator<IntTup>>> getTestCases()
    {
        return list(new Case<>(list(), i -> i.zip(iter(list())), list()),
                ...
                new Case<>(list(1, 2), i -> i.zip(iter(list(2))),
                        list(tup(1, 2))),
                new Case<>(list(1), i -> i.zip(iter(list(2, 3))),
                        list(tup(1, 2))));
    }
