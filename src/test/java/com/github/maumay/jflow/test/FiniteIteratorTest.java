...
public interface FiniteIteratorTest
{
            ...
            List<TestIterable<I>> iteratorProviders)
    {
        for (TestIterable<I> iteratorProvider : iteratorProviders) {
            ...
            assertForwardingAsExpected(expectedElements,
                    iteratorProvider.iter());
            assertNextElementChecksAsExpected(expectedElements,
                    iteratorProvider.iter());
            assertStandardIterationAsExpected(expectedElements,
                    iteratorProvider.iter());
            assertOptionalIterationAsExpected(expectedElements,
                    iteratorProvider.iter());
            assertUncheckedIterationAsExpected(expectedElements,
                    iteratorProvider.iter());
            assertAlternatingNextAndForwardCallsAsExpected(expectedElements,
                    iteratorProvider.iter());
        }
    }
    ...
    default Optional<?> nextOp(AbstractIterator iterator)
    {
        if (iterator instanceof AbstractRichIterator<?>) {
            ...
            return Option
                    .box(((AbstractDoubleIterator) iterator).nextDoubleOp());
        } else if (iterator instanceof AbstractLongIterator) {
    ...
    default void assertSizeDecreasesNyNextOpAsExpected(
    ...
    default void assertSizeDecreasesByForwardAsExpected(
            AbstractIterator iterator)
    {
        AbstractIteratorSize startSize = iterator.getSize().copy();
    ...
    default <T> void assertForwardingAsExpected(List<T> expectedElements,
            AbstractIterator iterator)
    {
    ...
    default void assertAlternatingNextAndForwardCallsAsExpected(
            List<?> expectedElements,
            AbstractIterator iterator)
    {
